...
% [a b] = check_box('list',{'a','b','c','d'},{1,0,1,0})
%
...
if useGUI,
    ...
    [nm, k] = local_GUImenu( xHeader, ArgsIn , ValIn);
else
...
%%#########################################################################
function [k, nm] = local_GUImenu( xHeader, xcItems , valItems )
...
%-------------------------------------------------------------------------
extraItems = 3;
numItems = length( xcItems ) + extraItems;
iALL     = numItems-2;
iCANCEL  = numItems-1;
iEND     = numItems;
...
for idx = numItems : -1 : (extraItems+1); % start from top of screen and go down
...
hBtn(iALL) = uicontrol('units'          ,MenuUnits, ...
               'position'       ,[uiBorder uiGap*1 textHigh textWide], ...
               'callback'       ,['set(gcf,''userdata'',',int2str(iALL),')'], ...
               'string'         ,'ALL' );
hBtn(iCANCEL)   = uicontrol('units'          ,MenuUnits, ...
               'position'       ,[uiBorder uiGap*1 textHigh textWide], ...
               'callback'       ,['set(gcf,''userdata'',',int2str(iCANCEL),')'], ...
               'string'         ,'CANCEL' );
hBtn(iEND)     = uicontrol('units'          ,MenuUnits, ...
               'position'       ,[uiBorder uiGap*1 textHigh textWide], ...
               'callback'       ,['set(gcf,''userdata'',',int2str(iEND),')'], ...
               'string'         ,'END' );
...
%------------------------------------------------------------------------
waitfor(hBtn(iEND),'value',1)

menuExist = figure_exist_figure(menuFig);

if( ~menuExist )
 op = zeros(1,numItems-extraItems);
elseif get(hBtn(iCANCEL),'value')  % cancel
  op = zeros(1,numItems-extraItems);
elseif get(hBtn(iALL),'value') % all
	op = ones(1,numItems-extraItems);
else                            % normale Auswahl
	for ii = 1:length(hBtn)-extraItems
