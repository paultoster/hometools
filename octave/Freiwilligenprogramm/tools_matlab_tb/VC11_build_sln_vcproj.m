function [okay,errtext] = VC11_build_sln_vcproj(vcsln_file,vcxproj_file,vcfilters_file ...
                                               ,use_vc_files,vc_sln_write,proj_name ...
                                               ,csrc_files,incl_dirs,preproc_defs,lib_files,lib_dirs)
%
% [okay,errtext] = VC11_build_sln_vcproj(vcsln_file,vcxproj_file,vcfilters_file ...
...
% all/src mit VC11_MEX_VPU_CAN_SIM.sln und VC11_MEX_VPU_CAN_SIM.vcproj
% gebildet
%
%
  okay           = 1;
  ...
  if( ~use_vc_files )
    ...
    sln_proto       = fullfile(s_file.dir,'src_modul','VC11_MEX_VPU_CAN_SIM.sln');
    vcxproj_proto   = fullfile(s_file.dir,'src_modul','VC11_MEX_VPU_CAN_SIM.vcxproj');
    vcfilters_proto = fullfile(s_file.dir,'src_modul','VC11_MEX_VPU_CAN_SIM.vcxproj.filters');
  ...
  [vcfilters_root,mm] = VC11_build_sln_vcprojfilter_add_sources(vcfilters_root,s);
  if( mm ),modifyflag = 1;end
  
  [vcx_root,mm]       = VC11_build_sln_vcproj_add_sources(vcx_root,s);
  if( mm ),modifyflag = 1;end
  %========================================================================
  % includes überprüfen und hinzufügen
  %----------------------------------
  [vcx_root,mm] = VC11_build_sln_vcproj_add_includes(vcx_root,incl_dirs,proj_dir);
  if( mm ),modifyflag = 1;end
  
  %========================================================================
  % preprocess_defs überprüfen und hinzufügen
  %----------------------------------
  [vcx_root,mm] = VC11_build_sln_vcproj_add_preprocessdef(vcx_root,preproc_defs);
  if( mm ),modifyflag = 1;end

  %========================================================================
  % lib_files überprüfen und hinzufügen
  %----------------------------------
  [vcx_root,mm] = VC11_build_sln_vcproj_add_lib_files(vcx_root,lib_files);
  if( mm ),modifyflag = 1;end

  %========================================================================
  % lib_dirs überprüfen und hinzufügen
  %----------------------------------
  [vcx_root,mm] = VC11_build_sln_vcproj_add_lib_dirs(vcx_root,lib_dirs,proj_dir);
  if( mm ),modifyflag = 1;end
...
function [vcfilters_root,modifyflag] = VC11_build_sln_vcprojfilter_add_sources(vcfilters_root,s)
  ...
  [vcfilters_root,okay,errtext,mm] = VC11_build_sln_vcprojfilter_add_filter_list(vcfilters_root,s);
  if( mm ),modifyflag = 1;end
  if( ~okay )
      tt = sprintf('Fehler in VC11_build_sln_vcproj_find_filter_list: \n%s',errtext);
      error('%s_error: %s',mfilename,tt);
  end
  [vcfilters_root,okay,errtext,mm] = VC11_build_sln_vcprojfilter_add_file_list(vcfilters_root,s);
  if( mm ),modifyflag = 1;end
  if( ~okay )
      tt = sprintf('Fehler in VC11_build_sln_vcproj_find_file_list: \n%s',errtext);
      error('%s_error: %s',mfilename,tt);
  end
%   for i = 1:n
...
%    ss_exist = VC11_build_sln_vcproj_get_path_struct(proj_dir,source_list);
%    ss_new   = VC11_build_sln_vcproj_get_path_struct(proj_dir,src_file_liste);
%    ss_add   = VC11_build_sln_vcproj_check_source_files(ss_exist,ss_new);
% 
%    if( ~isempty(ss_add) )
%     fchilds        = VC11_build_sln_vcproj_set_children(fchilds,ss_add);
...
%   % Suche die hinzuzufügenden Sourcen
%   ss_exist = VC11_build_sln_vcproj_get_path_struct(proj_dir,source_list);
%   ss_new   = VC11_build_sln_vcproj_get_path_struct(proj_dir,source_files);
%   ss_add   = VC11_build_sln_vcproj_check_source_files(ss_exist,ss_new);
%   
%   if( ~isempty(ss_add) )
%     fchilds        = VC11_build_sln_vcproj_set_children(fchilds,ss_add);
...
function [vcfilters_root,okay,errtext,modifyflag] = VC11_build_sln_vcprojfilter_add_filter_list(vcfilters_root,s)
  ...
  for i=1:length(s)
    ...
    if( isempty(ifound) )
      ...
      % unique Idetifier
      iadd = iadd + 1;
      uniqued = VC11_build_sln_vcprojfilter_build_unique_ident(data,iadd);
...
function uniqued = VC11_build_sln_vcprojfilter_build_unique_ident(data,iadd)
...
function [vcfilters_root,okay,errtext,modifyflag] = VC11_build_sln_vcprojfilter_add_file_list(vcfilters_root,s)
  modifyflag = 1;
...
function [vcx_root,modifyflag] = VC11_build_sln_vcproj_add_sources(vcx_root,s)
  % Sucht die den Zweig (Children) in 'VisualStudioProject.Files.Filter'
  % mit Attribut Name='sources'
  %
  modifyflag = 1;
...
function [vcx_root,modifyflag] = VC11_build_sln_vcproj_add_includes(vcx_root,include_list,proj_dir)
  ...
  for i=1:length(fchilds)
    if( strcmpi(fchilds(i).Name,'ClCompile') )
      ...
      [include_str,newflag] = VC11_build_sln_vcproj_check_include_list(data,include_list,proj_dir);
      if( newflag )
...
function [vcx_root,modifyflag] = VC11_build_sln_vcproj_add_preprocessdef(vcx_root,ppd_list)
  % Sucht die den Zweig (Children) in 'VisualStudioProject.Configurations.Configuration'
  % mit Attribut Name='AdditionalIncludeDirectories'
  %
  modifyflag = 0;
  ...
  for i=1:length(fchilds)
    if( strcmpi(fchilds(i).Name,'ClCompile') )
      ...
      %[ll,ivec] = xml_struct_get_list_childs_w_attribute(fchilds,'AdditionalIncludeDirectories');
      [ppd_str,newflag] = VC11_build_sln_vcproj_check_ppd_list(data,ppd_list);
      if( newflag )
...
function [vcx_root,modifyflag] = VC11_build_sln_vcproj_add_lib_files(vcx_root,lib_files)
  % Sucht die den Zweig (Children) in 'VisualStudioProject.Files.Filter'
  % mit Attribut Name='sources'
  %
  modifyflag = 0;
  ...
  for i=1:length(fchilds)
    if( strcmpi(fchilds(i).Name,'Link') )
      ...
      %[ppd_str,newflag] = VC11_build_sln_vcproj_check_ppd_list(data,ppd_list);
      [include_str,newflag] = VC11_build_sln_vcproj_check_lib_files_list(data,lib_files);
      if( newflag )
...
function [vcx_root,modifyflag] = VC11_build_sln_vcproj_add_lib_dirs(vcx_root,lib_dirs,proj_dir)
  % Sucht die den Zweig (Children) in 'VisualStudioProject.Configurations.Configuration'
  % mit Attribut Name='AdditionalIncludeDirectories'
  %
  modifyflag = 0;
  ...
  for i=1:length(fchilds)
    if( strcmpi(fchilds(i).Name,'Link') )
      ...
      [include_str,newflag] = VC11_build_sln_vcproj_check_lib_dirs(data,lib_dirs,proj_dir);
      if( newflag )
...
%==========================================================================
%==========================================================================
function ss = VC11_build_sln_vcproj_get_path_struct(proj_dir,ll)
  ss = [];
  n = length(ll);
  for i=1:n
...
function ss_add   = VC11_build_sln_vcproj_check_source_files(ss_exist,ss_new)
...
function children = VC11_build_sln_vcproj_set_children(children,ss_add)
...
function [include_string,newflag] = VC11_build_sln_vcproj_check_include_list(include_string,include_list,proj_dir)
%
% ss(i).abs_dir
% ss(i).rel_dir
%
...
function [ppd_string,newflag] = VC11_build_sln_vcproj_check_ppd_list(ppd_string,ppd_list)
%
%
  newflag = 0;
...
function [lib_files_string,newflag] = VC11_build_sln_vcproj_check_lib_files_list(lib_files_string,lib_files)
%
% ss(i).abs_dir
% ss(i).rel_dir
%
...
function [lib_dirs_string,newflag] = VC11_build_sln_vcproj_check_lib_dirs(lib_dirs_string,include_list,proj_dir)
%
% ss(i).abs_dir
% ss(i).rel_dir
%
