function q = build_vcproj_for_mex_sim_time_step_io(q)

  % Behandlung time_step_PROJ_NAME.m.m
  q = build_vcproj_for_mex_sim_time_step_io_m_modul(q);
  % Add dt to Parameterlist
  q = build_vcproj_for_mex_sim_time_step_add_parameter_dt(q);
  % Behandlung sim_TIME_STEP_SIM.cpp
  q = build_vcproj_for_mex_sim_time_step_io_sim_modul(q);
  % Behandlung fkt_TIME_STEP_SIM.cpp
  q = build_vcproj_for_mex_sim_time_step_io_fkt(q);
end
% Add dt to Parameterlist
function   q = build_vcproj_for_mex_sim_time_step_add_parameter_dt(q)

  flag = 1;
  for i=1:length(q.sim_def_var_par)
    if(  strcmp(q.sim_def_var_par(i).name,'dt') )
      flag = 0;
      break;
    end
  end
  ...
  if( flag )
    ...
    q.sim_def_var_par(npar+1).name      = 'dt';
    q.sim_def_var_par(npar+1).unit      = 's';
    q.sim_def_var_par(npar+1).type      = 'single';
    q.sim_def_var_par(npar+1).varname   = 'SimMod.dt';
    q.sim_def_var_par(npar+1).vartype   = 'C++';
    q.sim_def_var_par(npar+1).varinc    = '';
    q.sim_def_var_par(npar+1).varformat = 'double';
    q.sim_def_var_par(npar+1).varunit   = 's';
    q.sim_def_var_par(npar+1).comment   = '';    
  end
 
end
function q = build_vcproj_for_mex_sim_time_step_io_sim_modul(q)

  % Kennzeichnung im Header 
  q.KENNUNG_SIM_START_H        = '/*##TIME_STEP_SIM_START_H##*/';
  q.KENNUNG_SIM_END_H          = '/*##TIME_STEP_SIM_END_H##*/';
  % Kennzeichnung im Include-Bereich 
  q.KENNUNG_SIM_START_INC      = '/*##TIME_STEP_SIM_START_INC##*/';
  q.KENNUNG_SIM_END_INC        = '/*##TIME_STEP_SIM_END_INC##*/';
  % Kennzeichnung im Inpput-Loop-Bereich (d.h. nach ausgeführter Loop-Berechnung)
  q.KENNUNG_SIM_START_INP_LOOP = '/*##TIME_STEP_SIM_START_INP_LOOP##*/';
  q.KENNUNG_SIM_END_INP_LOOP   = '/*##TIME_STEP_SIM_END_INP_LOOP##*/';
  % Kennzeichnung im Output-Loop-Bereich (d.h. nach ausgeführter Loop-Berechnung)
  q.KENNUNG_SIM_START_OUT_LOOP = '/*##TIME_STEP_SIM_START_OUT_LOOP##*/';
  q.KENNUNG_SIM_END_OUT_LOOP   = '/*##TIME_STEP_SIM_END_OUT_LOOP##*/';
  ...
  q.SIM_KENNUNG_SIM_N_INP          = 'SIM_TIME_STEP_SIM_N_INP';
  q.SIM_KENNUNG_SIM_STRUCT_INP_VAR = 'SimInp';
  q.SIM_KENNUNG_SIM_N_OUT          = 'SIM_TIME_STEP_SIM_N_OUT';
  ...
  for i=1:length(q.source_files)
    if(str_find_f(q.source_files{i}{1},'sim_TIME_STEP_SIM.cpp'))
  ...
  if( isempty(simcppfile) )
    error(' Die Datei sim_TIME_STEP_SIM.cpp konnte in q.source_files nicht gefunden werden');
  end
  ...
  q.KENNUNG_SIM_START_PAR_PERM = '/*##TIME_STEP_SIM_START_PAR_PERM##*/';
  q.KENNUNG_SIM_END_PAR_PERM   = '/*##TIME_STEP_SIM_END_PAR_PERM##*/';
  % Kennzeichnung im Loop-Bereich (d.h. vor Loop-Berechnung)
  q.KENNUNG_SIM_START_PAR_LOOP = '/*##TIME_STEP_SIM_START_PAR_LOOP##*/';
  q.KENNUNG_SIM_END_PAR_LOOP   = '/*##TIME_STEP_SIM_END_PAR_LOOP##*/';
  % Kennzeichnung im Init-Bereich vor ausführung init
  q.KENNUNG_SIM_START_PAR_INIT0 = '/*##TIME_STEP_SIM_START_PAR_INIT0##*/';
  q.KENNUNG_SIM_END_PAR_INIT0   = '/*##TIME_STEP_SIM_END_PAR_INIT0##*/';
  % Kennzeichnung im Init-Bereich nach ausführung init
  q.KENNUNG_SIM_START_PAR_INIT1 = '/*##TIME_STEP_SIM_START_PAR_INIT1##*/';
  q.KENNUNG_SIM_END_PAR_INIT1   = '/*##TIME_STEP_SIM_END_PAR_INIT1##*/';

  q.VAR_SIM_PAR_NAME                  = 'pSimParVarNames';
  q.VAR_SIM_PAR_UNIT_NAME             = 'pSimParUnitNames';

  q.SIM_KENNUNG_SIM_N_PAR      = 'SIM_TIME_STEP_SIM_N_PAR';
  q.SIM_KENNUNG_SIM_N_LOOP_PAR = 'SIM_TIME_STEP_SIM_N_LOOP_PAR';
...
% simhfile bearbeiten
% sim_TIME_STEP_SIM.h          q.type='time_step'
%
  % h-Datei einlesen
  %-----------------
...
% sim_TIME_STEP_SIM.cpp          q.type='time_step'
