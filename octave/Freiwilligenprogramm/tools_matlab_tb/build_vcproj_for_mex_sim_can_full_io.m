function q = build_vcproj_for_mex_sim_can_full_io(q)

  % Behandlung sim_VPU_CAN_SIM.cpp
  q = build_vcproj_for_mex_sim_can_full_io_sim_modul(q);
  % Behandlung fkt_VPU_CAN_SIM.cpp
  q = build_vcproj_for_mex_sim_can_full_io_fkt(q);
end
function q = build_vcproj_for_mex_sim_can_full_io_sim_modul(q)
...
% sim_EMODUL_SIM.h         q.type='emodul'
...
%
  ...
  for i=1:length(q.sim_def_var_out)
    ...
    elseif( strcmp(q.sim_def_var_out(i).type,'mBuffer') )
      ...
      nout = nout + 1; % Counter 
    elseif( strcmp(q.sim_def_var_out(i).type,'vector') )
      nout = nout + 1;
    else
...
% q.sim_def_var_out(i).comment   Kommentar
  ...
  for ii=1:length(q.sim_def_var_out)
    ...
    elseif( strcmp(q.sim_def_var_out(ii).type,'vector') )
      i = i + 1;
      ifound               = str_find_f(q.sim_def_var_out(ii).varname,'(','vs');
      vecname              = q.sim_def_var_out(ii).varname(1:ifound-1);
      
      c_text               = str_get_quot_f(q.sim_def_var_out(ii).varname,'(',')','i');
      nvecname             = c_text{1};
        
%       ifound               = str_find_f(q.sim_def_var_out(ii).varformat,'(','vs');
%       vecformat            = q.sim_def_var_out(ii).varformat(1:ifound-1);
      
      c_text               = str_get_quot_f(q.sim_def_var_out(ii).varformat,'(',')','i');
      nvecformat           = c_text{1};
      
      [fac,offset,errtext] = get_unit_convert_fac_offset(q.sim_def_var_out(ii).varunit ...
                                                          ,q.sim_def_var_out(ii).unit);
      if( ~isempty(errtext) )
        error('%s: %s',q.sim_def_var_out(ii).name,errtext);
      end
      if( (abs(fac-1.0) < eps) && (abs(offset-0.0) < eps) )
      ...
      cinsert{ninsert+6} = sprintf('%s  %s i;' ...
                                   ,tt ...
                                   ,nvecformat ...
                                   );
      cinsert{ninsert+7} = sprintf('%s  for(i=0;i<%s;i++)' ...
