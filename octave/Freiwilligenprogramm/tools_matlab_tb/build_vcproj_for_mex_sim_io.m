function q = build_vcproj_for_mex_sim_io(q)

  if( strcmp(q.type,'can_full') )
    % Kennzeichnung im Header 
    q.KENNUNG_SIM_START_H        = '/*##VPU_CAN_SIM_START_H##*/';
    q.KENNUNG_SIM_END_H          = '/*##VPU_CAN_SIM_END_H##*/';
    % Kennzeichnung im Include-Bereich 
    q.KENNUNG_SIM_START_INC      = '/*##VPU_CAN_SIM_START_INC##*/';
    q.KENNUNG_SIM_END_INC        = '/*##VPU_CAN_SIM_END_INC##*/';
    % Kennzeichnung im Output-Loop-Bereich (d.h. nach ausgeführter Loop-Berechnung)
    q.KENNUNG_SIM_START_OUT_LOOP = '/*##VPU_CAN_SIM_START_OUT_LOOP##*/';
    q.KENNUNG_SIM_END_OUT_LOOP   = '/*##VPU_CAN_SIM_END_OUT_LOOP##*/';
    % Kennzeichnung im Output-ID-Bereich (d.h. direkt nach EInlesen der Botschaft mit ID)  
    q.KENNUNG_SIM_START_OUT_ID   = '/*##VPU_CAN_SIM_START_OUT_ID##*/';
    q.KENNUNG_SIM_END_OUT_ID     = '/*##VPU_CAN_SIM_END_OUT_ID##*/';

    q.VAR_SIM_OUT_NAME               = 'pSimOutVarNames';
    q.VAR_SIM_OUT_UNIT_NAME          = 'pSimOutUnitNames';
    q.SIM_KENNUNG_SIM_N_OUT          = 'SIM_VPU_CAN_SIM_N_OUT';
    q.SIM_KENNUNG_SIM_STRUCT_OUT_VAR = 'SimOut';
    
    % sim_VPU_CAN_SIM.cpp suchen
    %---------------------------
    simcppfile = '';
    for i=1:length(q.source_files)
      if(str_find_f(q.source_files{i}{1},'sim_VPU_CAN_SIM.cpp'))
    ...
    if( isempty(simcppfile) )
      error(' Die DAtei sim_VPU_CAN_SIM.cpp konnte in q.source_files nicht gefunden werden');
    end
  elseif( strcmp(q.type,'modul') )
    % Kennzeichnung im Header 
    q.KENNUNG_SIM_START_H        = '/*##MODUL_SIM_START_H##*/';
    q.KENNUNG_SIM_END_H          = '/*##MODUL_SIM_END_H##*/';
    % Kennzeichnung im Include-Bereich 
    q.KENNUNG_SIM_START_INC      = '/*##MODUL_SIM_START_INC##*/';
    q.KENNUNG_SIM_END_INC        = '/*##MODUL_SIM_END_INC##*/';
    % Kennzeichnung im Inpput-Loop-Bereich (d.h. nach ausgeführter Loop-Berechnung)
    q.KENNUNG_SIM_START_INP_LOOP = '/*##MODUL_SIM_START_INP_LOOP##*/';
    q.KENNUNG_SIM_END_INP_LOOP   = '/*##MODUL_SIM_END_INP_LOOP##*/';
    % Kennzeichnung im Output-Loop-Bereich (d.h. nach ausgeführter Loop-Berechnung)
    q.KENNUNG_SIM_START_OUT_LOOP = '/*##MODUL_SIM_START_OUT_LOOP##*/';
    q.KENNUNG_SIM_END_OUT_LOOP   = '/*##MODUL_SIM_END_OUT_LOOP##*/';
    ...
    q.SIM_KENNUNG_SIM_N_INP          = 'SIM_MODUL_SIM_N_INP';
    q.SIM_KENNUNG_SIM_STRUCT_INP_VAR = 'SimInp';
    q.SIM_KENNUNG_SIM_N_OUT          = 'SIM_MODUL_SIM_N_OUT';
    q.SIM_KENNUNG_SIM_STRUCT_OUT_VAR = 'SimOut';
    ...
    simcppfile = '';
    for i=1:length(q.source_files)
      if(str_find_f(q.source_files{i}{1},'sim_MODUL_SIM.cpp'))
        simcppfile = q.source_files{i}{1};
        simhfile = str_filename_change_ext(simcppfile,'h');
        break;
      end
    ...
    if( isempty(simcppfile) )
      error(' Die DAtei sim_MODUL_SIM.cpp konnte in q.source_files nicht gefunden werden');
    end
    
  else
    error('Der Typ ist noch nicht festgelegt !!!');
  end
  ...
  if( strcmp(q.type,'can_full') )
    ...
    q.KENNUNG_SIM_START_PAR_PERM = '/*##VPU_CAN_SIM_START_PAR_PERM##*/';
    q.KENNUNG_SIM_END_PAR_PERM   = '/*##VPU_CAN_SIM_END_PAR_PERM##*/';
    % Kennzeichnung im Loop-Bereich (d.h. vor Loop-Berechnung)
    q.KENNUNG_SIM_START_PAR_LOOP = '/*##VPU_CAN_SIM_START_PAR_LOOP##*/';
    q.KENNUNG_SIM_END_PAR_LOOP   = '/*##VPU_CAN_SIM_END_PAR_LOOP##*/';
    % Kennzeichnung im Init-Bereich vor ausführung init
    q.KENNUNG_SIM_START_PAR_INIT0 = '/*##VPU_CAN_SIM_START_PAR_INIT0##*/';
    q.KENNUNG_SIM_END_PAR_INIT0   = '/*##VPU_CAN_SIM_END_PAR_INIT0##*/';
    % Kennzeichnung im Init-Bereich nach ausführung init
    q.KENNUNG_SIM_START_PAR_INIT1 = '/*##VPU_CAN_SIM_START_PAR_INIT1##*/';
    q.KENNUNG_SIM_END_PAR_INIT1   = '/*##VPU_CAN_SIM_END_PAR_INIT1##*/';
  else
    % Kennzeichnung im Loop-const-Bereich (d.h. vor Loop-Berechnung)
    q.KENNUNG_SIM_START_PAR_PERM = '/*##MODUL_SIM_START_PAR_PERM##*/';
    q.KENNUNG_SIM_END_PAR_PERM   = '/*##MODUL_SIM_END_PAR_PERM##*/';
    % Kennzeichnung im Loop-Bereich (d.h. vor Loop-Berechnung)
    q.KENNUNG_SIM_START_PAR_LOOP = '/*##MODUL_SIM_START_PAR_LOOP##*/';
    q.KENNUNG_SIM_END_PAR_LOOP   = '/*##MODUL_SIM_END_PAR_LOOP##*/';
    % Kennzeichnung im Init-Bereich vor ausführung init
    q.KENNUNG_SIM_START_PAR_INIT0 = '/*##MODUL_SIM_START_PAR_INIT0##*/';
    q.KENNUNG_SIM_END_PAR_INIT0   = '/*##MODUL_SIM_END_PAR_INIT0##*/';
    % Kennzeichnung im Init-Bereich nach ausführung init
    q.KENNUNG_SIM_START_PAR_INIT1 = '/*##MODUL_SIM_START_PAR_INIT1##*/';
    q.KENNUNG_SIM_END_PAR_INIT1   = '/*##MODUL_SIM_END_PAR_INIT1##*/';
  end
  q.VAR_SIM_PAR_NAME                  = 'pSimParVarNames';
  q.VAR_SIM_PAR_UNIT_NAME             = 'pSimParUnitNames';

  if( strcmp(q.type,'can_full') )
    q.SIM_KENNUNG_SIM_N_PAR      = 'SIM_VPU_CAN_SIM_N_PAR';
    q.SIM_KENNUNG_SIM_N_LOOP_PAR = 'SIM_VPU_CAN_SIM_N_LOOP_PAR';
  else
    q.SIM_KENNUNG_SIM_N_PAR      = 'SIM_MODUL_SIM_N_PAR';
    q.SIM_KENNUNG_SIM_N_LOOP_PAR = 'SIM_MODUL_SIM_N_LOOP_PAR';
  end
...
% simhfile bearbeiten
% sim_MODUL_SIM.h          q.type='modul'
% sim_VPU_CAN_SIM.h        q.type='can_full'
%
  % h-Datei einlesen
  %-----------------
...
% sim_MODUL_SIM.cpp          q.type='modul'
% sim_VPU_CAN_SIM.cpp        q.type='can_full'
  ...
  % Ausgabe-ID-Bereich bearbeiten
  %---------------------------------
  if( strcmp(q.type,'can_full') )

    [c,i0] = Kennung_bereinigen(c,q.KENNUNG_SIM_START_OUT_ID,q.KENNUNG_SIM_END_OUT_ID,1);

    cinsert = {};
    ninsert = 0;
    % Zuordnung Werte  
    [cinsert,ninsert] = build_vcproj_for_mex_sim_io_build_sim_file_output_id(q,cinsert,ninsert);
  ...
  %---------------------------------
  if( strcmp(q.type,'modul') || strcmp(q.type,'can_full') )
...
%------------------------------------------------------------------
  ...
  for i = 1:n
    if( ~isempty(q.sim_def_var_inp(i).varinc) )
      [ifound,ipos] = cell_find_f(include_files,q.sim_def_var_inp(i).varinc,'fl');
      if( isempty(ifound) )
        include_files = cell_add(include_files,q.sim_def_var_inp(i).varinc);
  ...
  for i = 1:n
    if( ~isempty(q.sim_def_var_out(i).varinc) )
      [ifound,ipos] = cell_find_f(include_files,q.sim_def_var_out(i).varinc,'fl');
      if( isempty(ifound) )
        include_files = cell_add(include_files,q.sim_def_var_out(i).varinc);
  ...
  for i = 1:n
    if( ~isempty(q.sim_def_var_par(i).varinc) )
      [ifound,ipos] = cell_find_f(include_files,q.sim_def_var_par(i).varinc,'fl');
      if( isempty(ifound) )
        include_files = cell_add(include_files,q.sim_def_var_par(i).varinc);
...
function  [cinsert,ninsert] = build_vcproj_for_mex_sim_io_build_sim_file_h(q,ninp,nout,npar,nlooppar,cinsert,ninsert)
  ...
  if( strcmp(q.type,'can_full') || strcmp(q.type,'modul') )
    ninsert = ninsert+1;
    if( nout )
...
% 
%     % Inpt Time
%      cinsert{ninsert+1} = 'typedef struct';
%      cinsert{ninsert+2} = '{';
%      cinsert{ninsert+3} = '  double      *ptime;';
%      cinsert{ninsert+4} = '  size_t      ntime;';
%      cinsert{ninsert+5} = '  bool        found;';
%      cinsert{ninsert+6} = sprintf('}S%sTime;',q.SIM_KENNUNG_SIM_STRUCT_INP_VAR);
%      cinsert{ninsert+7} = sprintf('extern S%sTime %sTime;' ...
...
%      cinsert{ninsert+1} = 'typedef struct';
%      cinsert{ninsert+2} = '{';
%      cinsert{ninsert+3} = '  double      *pvec;';
%      cinsert{ninsert+4} = '  size_t      nvec;';
%      cinsert{ninsert+5} = '  std::string vecname;';
%      cinsert{ninsert+6} = '  bool        found;';
%      cinsert{ninsert+7} = sprintf('}S%s;',q.SIM_KENNUNG_SIM_STRUCT_INP_VAR);
%      cinsert{ninsert+8} = sprintf('extern S%s %s[%s];' ...
%                                  ,q.SIM_KENNUNG_SIM_STRUCT_INP_VAR ...
%                                  ,q.SIM_KENNUNG_SIM_STRUCT_INP_VAR ...
%                                  ,q.SIM_KENNUNG_SIM_N_INP ...
%                                  );
% 
%     ninsert             = ninsert + 8;
%   end
...
%     cinsert{ninsert+1} = 'typedef struct';
%     cinsert{ninsert+2} = '{';
%     cinsert{ninsert+3} = '  std::vector<double> time;';
%     cinsert{ninsert+4} = '  std::vector<double> vec;';
%     cinsert{ninsert+5} = '  std::string         comment;';
%     cinsert{ninsert+6} = sprintf('}S%s;',q.SIM_KENNUNG_SIM_STRUCT_OUT_VAR);
%     cinsert{ninsert+7} = sprintf('extern S%s %s[%s];' ...
%                                ,q.SIM_KENNUNG_SIM_STRUCT_OUT_VAR ...
%                                ,q.SIM_KENNUNG_SIM_STRUCT_OUT_VAR ...
%                                ,q.SIM_KENNUNG_SIM_N_OUT ...
%                                );
% 
%     ninsert             = ninsert + 7;
%   end
...
% function [cinsert,ninsert] = build_vcproj_for_mex_sim_output_build_sim_file_out_struct(q,cinsert,ninsert)  
% %   if( strcmp(q.type,'modul') )
% %     ninsert = ninsert+1;
% %     cinsert{ninsert} = sprintf('S%sTime %sTime;',q.SIM_KENNUNG_SIM_STRUCT_INP_VAR ...
% %                                             ,q.SIM_KENNUNG_SIM_STRUCT_INP_VAR ...
% %                                             );
% %     ninsert = ninsert+1;
% %     cinsert{ninsert} = sprintf('S%s %s[%s];',q.SIM_KENNUNG_SIM_STRUCT_INP_VAR ...
% %                                             ,q.SIM_KENNUNG_SIM_STRUCT_INP_VAR ...
% %                                             ,q.SIM_KENNUNG_SIM_N_INP ...
% %                                             );
% %   end
%   if( strcmp(q.type,'can_full') )
%     ninsert = ninsert+1;
%     cinsert{ninsert} = sprintf('S%s %s[%s];',q.SIM_KENNUNG_SIM_STRUCT_OUT_VAR ...
%                                             ,q.SIM_KENNUNG_SIM_STRUCT_OUT_VAR ...
%                                             ,q.SIM_KENNUNG_SIM_N_OUT ...
%                                             );
%   end
...
% q.sim_def_var_inp(i).comment   Kommentar
  ...
  if( mBuffer_flag )
    cinsert{ninsert+3} = [tt,'size_t ndval,i;'];
  else
    cinsert{ninsert+3} = [tt,'size_t ndval;'];
  end
  ninsert            = ninsert+3;
  i = 0;
  for ii=1:length(q.sim_def_var_inp)
    ...
    if( strcmp(q.sim_def_var_inp(ii).type,'single') )
      ...
      cinsert{ninsert+4} = sprintf('%sndval = %s[%i].nvec[0];' ...
                                  ,tt ...
                                  ,q.SIM_KENNUNG_SIM_STRUCT_INP_VAR ...
                                  ,(i-1) ...
                                  );
      if( nofacoffset )
        cinsert{ninsert+5} = sprintf('%s%s = (%s)(pdval[MIN(itime,ndval-1)]);' ...
                                    ,tt ...
                                    ,q.sim_def_var_inp(ii).varname ...
                                    ,q.sim_def_var_inp(ii).varformat ...
                                    );
      else
        cinsert{ninsert+5} = sprintf('%s%s = (%s)(pdval[MIN(itime,ndval-1)] * %f + %f);' ...
                                    ,tt ...
                                    ,q.sim_def_var_inp(ii).varname ...
                                    ,q.sim_def_var_inp(ii).varformat ...
                                    ,fac ...
    ...
    elseif( strcmp(q.sim_def_var_inp(ii).type,'mBuffer') )
      for j=1:length(q.sim_def_var_inp(ii).vecnames)
        ...
        cinsert{ninsert+3} = sprintf('%spdval = %s[%i].pvec[MIN(itime,%s[%i].ncells-1)];' ...
                                    ,tt ...
                                    ,q.SIM_KENNUNG_SIM_STRUCT_INP_VAR ...
                                    ,(i-1) ...
                                    ,q.SIM_KENNUNG_SIM_STRUCT_INP_VAR ...
        ...
        cinsert{ninsert+4} = sprintf('%sndval = %s[%i].nvec[MIN(itime,%s[%i].ncells-1)];' ...
                                    ,tt ...
                                    ,q.SIM_KENNUNG_SIM_STRUCT_INP_VAR ...
                                    ,(i-1) ...
                                    ,q.SIM_KENNUNG_SIM_STRUCT_INP_VAR ...
        ...
        cinsert{ninsert+5} = sprintf('%sfor(i=0;i<MIN(%i,ndval);i++)' ...
                                    ,tt ...
                                    ,q.sim_def_var_inp(ii).length ...
                                    );
        cinsert{ninsert+6} = sprintf('%s{',tt);
        if( nofacoffset )
          cinsert{ninsert+7} = sprintf('%s  %s[i] = (%s)(pdval[i]);' ...
                                      ,tt ...
                                      ,varname ...
                                      ,varformat ...
                                      );
        else
          cinsert{ninsert+7} = sprintf('%s  %s[i] = (%s)(pdval[i] * %f + %f);' ...
                                      ,tt ...
                                      ,varname ...
                                      ,varformat ...
                                      ,fac ...
...
function [cinsert,ninsert] = build_vcproj_for_mex_sim_io_build_sim_file_output_id(q,cinsert,ninsert)
...
% q.sim_def_var_out(i).comment   Kommentar
  ...
  s = [];
  for i=1:length(q.sim_def_var_out)
    if( q.sim_def_var_out(i).id ) 
      if( strcmp(q.sim_def_var_out(i).type,'single') )
        ii = ii+1;
        s(ii).unit    = q.sim_def_var_out(i).unit;
        s(ii).varunit = q.sim_def_var_out(i).varunit;
        s(ii).name    = q.sim_def_var_out(i).name;
        s(ii).id      = q.sim_def_var_out(i).id;
        s(ii).channel = q.sim_def_var_out(i).channel;
        s(ii).varname = q.sim_def_var_out(i).varname;
        s(ii).comment = q.sim_def_var_out(i).comment;
      elseif( strcmp(q.sim_def_var_out(i).type,'mBuffer') )
        error('%s_errror: type=%s muss hier angepasst werden',q.sim_def_var_out(i).type);
      else
        error('%s_errror: type=%s ist nicht bekannt',q.sim_def_var_out(i).type);
      end
    end
  ...
  for i=1:length(s)
    if( s(i).id )  
      ...
      [fac,offset,errtext] = get_unit_convert_fac_offset(s(i).unit ...
                                                        ,s(i).varunit);
      if( ~isempty(errtext) )
        error('%s: %s',s(i).name,errtext);
      end
      if( (abs(fac-1.0) < eps) && (abs(offset-0.0) < eps) )
        nofacoffset = 1;
      else
      ...
      cinsert{ninsert+1} = '';
      cinsert{ninsert+2} = sprintf('%s/* %s */',tt,s(i).name);
      if( i == 1 )
        cinsert{ninsert+3} = sprintf('%sif(      (p_rec_buf->id == %i ) && (p_rec_buf->channel == %i) )' ...
                                    ,tt,s(i).id,s(i).channel-1);
      else
        cinsert{ninsert+3} = sprintf('%selse if( (p_rec_buf->id == %i ) && (p_rec_buf->channel == %i) )' ...
                                    ,tt,s(i).id,s(i).channel-1);
      end
      cinsert{ninsert+4} = sprintf('%s{',tt);
      
      cinsert{ninsert+5} = sprintf('%s%s[%i].time.push_back(*ptime);' ...
                                  ,tttt,q.SIM_KENNUNG_SIM_STRUCT_OUT_VAR,(i-1));
      if( nofacoffset )
        cinsert{ninsert+6} = sprintf('%s%s[%i].vec.push_back(%s);' ...
                                    ,tttt,q.SIM_KENNUNG_SIM_STRUCT_OUT_VAR,(i-1) ...
                                    ,s(i).varname ...
                                    );
      else
        cinsert{ninsert+6} = sprintf('%s%s[%i].vec.push_back(%s*%f+%f);' ...
                                    ,tttt,q.SIM_KENNUNG_SIM_STRUCT_OUT_VAR,(i-1) ...
                                    ,s(i).varname ...
                                    ,fac, offset ...
                                    );
      end        
      cinsert{ninsert+7} = sprintf('%s%s[%i].comment = "%s";' ...
                                  ,tttt,q.SIM_KENNUNG_SIM_STRUCT_OUT_VAR,(i-1) ...
                                  ,s(i).comment);
      cinsert{ninsert+8} = sprintf('%s%s[%i].isvecofvec = 0;' ...
  ...
  if( flag )
    ...
    for ii=1:length(q.sim_def_var_par)
      ...
      elseif( strcmp(q.sim_def_var_par(ii).type,'cinit1') )
        ...
        cinsert{ninsert+2} = sprintf('%s/* %s */',tt,q.sim_def_var_par(ii).name);
        if( nofacoffset )
          cinsert{ninsert+4} = sprintf('%s%s = (%s)(%s);' ...
                                      ,tt ...
                                      ,q.sim_def_var_par(ii).varname ...
                                      ,q.sim_def_var_par(ii).varformat ...
                                      ,q.sim_def_var_par(ii).varval ...
          ...
          cinsert{ninsert+4} = sprintf('%s%s = (%s)(%s * %f + %f);' ...
                                      ,tt ...
                                      ,q.sim_def_var_par(ii).varname ...
                                      ,q.sim_def_var_par(ii).varformat ...
                                      ,q.sim_def_var_par(ii).varval ...
