...
function s_duh = duh_das_daten_wandeln_f(s_duh)
%
% Daten einlesen
%
%
...
n_data = duh_das_daten_wandeln_do_f(s_liste(1).c_value,s_liste(2).c_value,s_liste(3).c_value{1}, ...
                                s_liste(4).c_value{1},s_liste(6).c_value,s_liste(7).c_value{1},s_liste(8).c_value{1});
end_time = cputime;
...
function   n_data = duh_das_daten_wandeln_do_f(c_dir, c_prc_files, peak_filt_flag, ...
                                           peak_filt_std_fac, c_my_func_file,output_format,insert_name)
...
s_files = suche_files_ext_f(c_sub,'dat');
...
for i=1:length(s_files)
    ...
    n_data = n_data+1;
    [okay,s_data] = duh_das_daten_einlesen_f(s_files(i).fullname,c_prc_files);
