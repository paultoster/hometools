function  s_duh = duh_daten_bearb_lbf(data_selection,s_duh)

s_frage    = 0;
s_liste = o_abfragen_werte_liste_erstellen_f ...
             (1      ,'signal'                  ,''       ,'Mit welchem Signal(e) filtern' ...
             ,1      ,'time_signal'             ,0        ,'Zeitsignal' ...
             ,1      ,'cutoff_freq'             ,0        ,'Eckfrequenz' ...
...
%                    return;
            ...
            case 3            
				s_frage.c_comment{1} = 'unabhänigige-Variable:';
                for i=1:length(data_selection)
                    c_names   = fieldnames(s_duh.s_data(data_selection(i)).d);
                    t_name    = c_names{1};
                    t_unit    = s_duh.s_data(data_selection(i)).u.(t_name);
                    s_frage.c_comment{1+i} = sprintf('%i: %s %s  %s',i,s_duh.s_data(data_selection(i)).name,t_name,t_unit);
                end
				s_frage.frage     = 'Eckfrequenz angeben';
				s_frage.prot      = 0;
				s_frage.command   = 'cutoff_freq';
				s_frage.type      = 'double';
...
CutOffFreq     = s_liste(3).c_value{1};
...
for i=1:length(data_selection)
    ...
    %
    % Sample Frequency
    t = s_duh.s_data(i_data).d.(time_name);
    ...
    for j=1:length(c_names)
        ...
        for k=1:length(c_names_select)
            ...
            if( strcmp(c_names{j},c_names_select{k}) )
                ...
                vektor = lbf_filter(s_duh.s_data(i_data).d.(c_names{j}),CutOffFreq,SampleFreq,zp_flag);
