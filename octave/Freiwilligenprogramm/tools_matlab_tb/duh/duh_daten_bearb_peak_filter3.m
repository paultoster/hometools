...
function s_duh = duh_daten_bearb_peak_filter3(selection,s_duh)

s_frage.c_comment{1} = 'Peakfilter3: aus diff(data) wird die Standardabweichung (std) bestimmt';
s_frage.c_comment{2} = '            Stellen mit n Werten exakt null und hohe Gradienten am Rand werden erkannt (differenz > faktor*std)';
...
for i=1:length(selection)
    i1 = selection(i);
    [s_duh.s_data(i1).d,c_comment] = peak_filter3_f(s_duh.s_data(i1).d,s_duh.s_einstell.peak_filt_std_fac);
