...
s_liste = o_abfragen_werte_liste_erstellen_f ...
             ...
             ,1      ,'offset'           ,0        ,'Zeitoffset nach vorne (-) nach hinten (+) verschieben zum Anschneiden' ...
             );
option_flag = 1;
...
%                    return;
            ...
            case 4 % peakfilter faktor
				...
				s_frage.frage     = 'Offset nach vorne (negativ) nach hinten (positiv) zum Abschneiden';
				s_frage.prot      = 0;
        ...
        if( okay )
            ...
            for j=1:length(c_names)
                ...
                if( n_act == nend )
                    ...
                    command = sprintf('s_duh.s_data(i_data).d.%s = s_duh.s_data(i_data).d.%s(1:index);',char(c_names{j}),char(c_names{j}));
                    eval(command);
