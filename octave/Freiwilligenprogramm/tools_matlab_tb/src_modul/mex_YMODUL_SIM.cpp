//--------------------------------------------------------------
// file: mex_YMODUL_SIM.cpp - Matlab Mex function application
//--------------------------------------------------------------
/*##FUNCTION_CODE_NAMEXYZ_START##*/
/*##FUNCTION_CODE_NAMEXYZ_END##*/
#include "mex.h"

#include "sim_YMODUL_SIM.h"
...
std::string EcalMeasPath = "";        // Filename mit Messung
...
void mexFunction( int nlhs, mxArray *plhs[], int nrhs, const mxArray  *prhs[] )
{
  ...
  if( nrhs == 0 )
  {
    mexPrintf("e = %s(EcalMeasPath]);\n",MEX_FUNCTION_NAME);
    mexPrintf("EcalMeasPath            char *      Name of measurement Path\n");
  ...
  // 1. Parameter EcalMeasPath lesen
  //================================
	irhs = 0;
  if( !mxIsChar(prhs[irhs]) )
  {
		sprintf_s(errtext,lerrtext,"Error_%s: Erster Parameter ist kein (char) (EcalMeasPath)",MEX_FUNCTION_NAME);
  ...
  else 
  {
		...
		if( ptext != 0 ) 
		{
			...
			mxGetString(prhs[i],ptext,(mxGetM(prhs[i]) * mxGetN(prhs[i])+1));
		}
    else
    {
		  sprintf_s(errtext,lerrtext,"Error_%s: malloc char * %i konnte nicht erzeugt werden\n",MEX_FUNCTION_NAME,(mxGetM(prhs[i]) * mxGetN(prhs[i])+1));
      mexErrMsgTxt(errtext);
		  return;
		}
		EcalMeasPath = ptext;
