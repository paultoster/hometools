...
elseif strcmp(state,'move') % Execute the WindowButtonMotionFcn
    ...
    if ~isempty(gco)
        ...
        if strcmp(htype,'text')             %The text
            ...
            handles = get_user_data_datalabel(gco);
            ph = handles(2);
...
function deleteObject_cb(o,e)
  ud = get_user_data_datalabel(gco);
  delete([gco ud(2)]);
function data = get_user_data_datalabel(fid)
% Userdaten aus Figure/Diagramm holen
% datalabel ist Kennzeichnung gegenüber anderen Daten von anderen Programmen
  FH = get(fid,'userdata');
  
  if(  ~isempty(FH) && isfield(FH,'datalabel') )
    data = FH.datalabel;
  else
    data = [];
  end
...
function set_user_data_datalabel(fid,data)
