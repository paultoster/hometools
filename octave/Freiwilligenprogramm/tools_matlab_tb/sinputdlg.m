function Answer=sinputdlg(Prompt, Title, NumLines, DefAns, Resize)
...
set(InputFig,'Position',sgetnicedialoglocation(FigPos,get(InputFig,'Units')));
...
ssetdefaultbutton(InputFig, OKHandle);
...
set([InputFig, OKHandle, CancelHandle], 'keypressfcn', @doCallback);
...
function figure_size = sgetnicedialoglocation(figure_size, figure_units)
...
function ssetdefaultbutton(figHandle, btnHandle)
...
if (usejava('awt') == 1)
    ...
    suseJavaDefaultButton(figHandle, btnHandle)
else
    % We are running with Native Figures
    suseHGDefaultButton(figHandle, btnHandle);
end

function suseJavaDefaultButton(figH, btnH)
...
function suseHGDefaultButton(figHandle, btnHandle)
