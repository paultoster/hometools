function [okay,p] = figure_show_at_index(p,index,ccolor)
%
% okay = figure_show_at_index(p,index,color)
%
% okay = figure_show_at_index(p,index)
%
  ...
  for i=1:n
    ...
    if( p(i).type == 'i' && (index > 0) )
      ...
      for j=1:length(p(i).hline)
        ...
        index = max(1,min(index,length(xdata)));
