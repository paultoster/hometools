function q = build_vcproj_for_mex_sim_can_full_io_fkt(q)
  ...
  for i=1:length(q.source_files)
    if(str_find_f(q.source_files{i}{1},'fkt_VPU_CAN_SIM.c'))
      fktcfile = q.source_files{i}{1};
      fkthfile   = str_filename_change_ext(fktcfile,'h');
      break;
    end
  end
  if( isempty(fktcfile) )
    error(' Die Datei fkt_VPU_CAN_SIM.c konnte in q.source_files nicht gefunden werden');
  end

  % Einlesen
  [ okay,c,n ] = read_ascii_file(fktcfile);
  if( ~okay )
    error('%s_error: fktcfile: <%s> konnte nicht eingelesen werden !!!',mfilename,fktcfile);
  end
...
%                                oder text-Datei mit den End-Fkt-Zeilen
  ...
  %------------------
  okay = write_ascii_file(fktcfile,c);
  if( ~okay )
    error('%s_error: Fehler bei Schreiben von fktcfile: <%s>',mfilename,fktcfile);
  end
