...
case 0 % no arg in
 ...
 otherwise  % turn on
  ...
  set_user_data_drag(fignum,figstate);
  dragon(fignum);
...
figstate = get_user_data_drag(fignum);
dragreset(fignum);
...
set_user_data_drag(axishandle,state);


function updateaxes(axishandle,motion)
% state = get(axishandle,'UserData');
state = get_user_data_drag(axishandle);
...
function data = get_user_data_drag(fid)
% Userdaten aus Figure/Diagramm holen
% drag ist Kennzeichnung gegenüber anderen Daten von anderen Programmen
  FH = get(fid,'userdata');
  
  if(  ~isempty(FH) && isfield(FH,'drag') )
    data = FH.drag;
  else
    data = [];
  end
...
function set_user_data_drag(fid,data)
