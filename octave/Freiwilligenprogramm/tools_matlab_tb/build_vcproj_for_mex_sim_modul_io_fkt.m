function q = build_vcproj_for_mex_sim_modul_io_fkt(q)
  ...
  for i=1:length(q.source_files)
    if(str_find_f(q.source_files{i}{1},'fkt_MODUL_SIM.cpp'))
  ...
  if( isempty(fktcppfile) )
    error(' Die Datei fkt_MODUL_SIM.cpp konnte in q.source_files nicht gefunden werden');
  end
