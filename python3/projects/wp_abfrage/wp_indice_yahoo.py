...
def process_akt(wb_obj,indice):
    ...
    return (status,errtext)
# end def
def merge_indice_np_obj_new_to_np_obj(wb_obj,np_obj,np_obj_new):
    """

    :param df:
    :param df_new:
    :param dat_name:
    :param usdeuro_name: (status, errtext, np_obj_out) = obj.merge_usdeuro_dfnew_to_df(df,df_new, dat_name,usdeuro_name)
    :return:
    """
    status = hdef.OKAY
    errtext = ""
    ...
    if len(sort_index_list):
        ...
        for index,val in enumerate(sort_index_list):
            ...
            # end if
        # end for
        np_obj_out = wp_bearbeit.build_indice_np_obj(wb_obj,wb_obj.par.INDICES_USDEURO_NAME)
        np_obj_out.put_signal(np_dat_merge, np_usdeuro_merge)

    # end if
    return (status, errtext, np_obj_out )
# end def
def get_from_start_dat_to_end_dat(wb_obj, start_dat, end_dat,indice):
    """
    :param wb_obj:
    :param start_dat:
    :param end_dat:
    :return: (status, errtext,np_obj) = wp_base_usdeuro.get_from_start_dat_to_end_dat(wb_obj, start_dat, end_dat,indice)
    """
    status = hdef.OKAY
    errtext = ""
    ...
    if (last_dat == -1) or (first_dat == -1):
        ...
        errtext = f"get_from_start_dat_to_end_dat: Für die Währungsumrechnung {wb_obj.par.INDICES_USDEURO_NAME} sind keine Daten gespeichert!!"
    ...
    if end_dat > last_dat:
        ...
        (status, errtext) = process_akt(wb_obj,indice)
    ...
    # Stutze Vektoren auf den Indize-Bereich ein
        ...
        np_obj.put_signal(np_obj.dat_np_array[start_index:end_index+1],
                          np_obj.indice_np_array[start_index:end_index+1])
    # end if

    return (status, errtext, np_obj)
# end def
def get_act(wb_obj,indice):
    status = hdef.OKAY
    errtext = ""
