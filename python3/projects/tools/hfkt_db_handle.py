...
#                         DB_DATA_TYPE_CENT        Integer-Typ  as Cent
...
# zelle3                = Menge,                DB_DATA_TYPE_FLOAT,        [kg],      kg
# zelle4                = Anzahl,               DB_DATA_TYPE_INT,          [num],     Wert in Cent
# zelle5                = Herkunft,             DB_DATA_TYPE_STR,          [],        Herkunft des Materials
# zelle6                = Wert,                 DB_DATA_TYPE_CENT,         [Cent],    Wert in Cent
#
#
...
#                                               ,CELL_MENGE:                 {'type':hdb.DB_DATA_TYPE_FLOAT,'unit':'[kg]','comment':'Menge'} \
#                                               ,CELL_ANZAHL:                  {'type':hdb.DB_DATA_TYPE_INT,'unit':'[num]','comment':'Anzahl von ?'} \
#                                               ,CELL_HERKUNFT:              {'type':hdb.DB_DATA_TYPE_STR,'unit':'[]','comment':'Herkunft des Materials'} \
#                                               ,CELL_WERT:                  {'type':hdb.DB_DATA_TYPE_CENT,'unit':'[Cent]','comment':'Wert in Cent'} \
#                                               }
#                                      }
#                }
#
...
#                 .type             Schlüsselwörter Tabelle siehe oben
#                 .comment
...
#
# dbh.get_tab_header_list(self,tabname)
#
#    Abfrage der Headernamen in einer liste der Tabelle tabname
#    header_liste = hdb.get_tab_header_list(self,tabname)
#
...
#
#    Rückgabe des primkeys als liste für den Wert in dem Zellen von cellname
#    primkey_liste = hdb.get_tab_primary_key_with_value(self,tabname,cellname,value)
#
# dbh.get_tab_type_of_cell_name(tabname,cellname)
#
#    get type of table with tablename and for specific cellname,
#    if cellname is a list also return value is a list
#
...
#
# dbh.def get_tab_data_with_value_type_in_dict(tabname,cellname,value,header_liste=[]):
#
#   Von Tabelle tabname mit dem Zellenname = value alle Daten und Typen zurückgeben,
#   wenn header_liste leer, dann alles zurückgeben
#   tabname: name of table
#   cellname: name eine header in der Tabelle
#   value:    name des items dass unter den header zu finden ist
#     d = dbh.get_tab_data_with_value(tabname,cellname,value)
#     c = dbh.get_tab_data_with_value(tabname,cellname,value,header_liste)
#
#   d[header][0] = data
#   d[header][1] = type     hdb.DB_DATA_TYPE_STR         String-Typ
#                           hdb.DB_DATA_TYPE_FLOAT       Float-Typ
#                           hdb.DB_DATA_TYPE_INT         Integer-Typ
#                           hdb.DB_DATA_TYPE_KEY         Key-Typ       Key einer anderen Tabelle
#                           hdb.DB_DATA_TYPE_CENT        Inter-Typ für Cent
#
#
...
#
#-------------------------------------------------------------------------------------------
#  status = dbh.add_new_data_set(tabname(string),d(dict))
#
#   (d[cellname1]=item1,d[cellname2]=item2,...)
#
#-------------------------------------------------------------------------------------------
...
#
#---------------------------------------------------------------------------------------------
#  status = dbh.modify_data_by_primkey(tabname,primkey,d)
#
#    Aus d (dictionary) werden die items ausgelesen und gesetzt und eingeben
#    und mit dem primkey geändert
#    d[cellnameA]=WertA
#    d[cellnameB]=WertB
#    ...
#    Alle Zellen einer Tabelle müssen vorhanden sein
#    return status
#----------------------------------------------------------------------------------------------
#  staus = dbh.delete_data_by_primkey(tabname,primkey)
#
    ...
    for key in out:
      ...
      for index in indexliste:
        ...
        if( len(liste) > 0 ):
          ...
          # prüfen, ob Tabellen name zu z.B. key angeh�ngt ist
...
#-------------------------------------------------------------------------------
  ...
  def find_name_in_list(self,cell_name,cell_liste):
    """
    Prüft ob Cellname bereits voranden
    """
...
#-------------------------------------------------------------------------------
#-------------------------------------------------------------------------------
  def get_tab_header_list(self,tabname,type):
    """
      Abfrage header namens liste

      header_liste = hdb.get_tab_header_list(self,tabname,type)

      tabname             Tabellen Name
      type = 1            ohne primekey
    ...
    primekey = self.get_tab_primary_key_name(tabname)
    header_liste = []
    cells = self.db.get_db_table_info(tabname)

    for items in cells:
      if( type == 1 ):
        if( items[0] != primekey):
          header_liste.append(items[0])
        #endif
      else:
        header_liste.append(items[0])
      #endif
...
#-------------------------------------------------------------------------------
#-------------------------------------------------------------------------------
  def get_tab_type_of_cell_name(self,tabname,cellname):
    """
     get type of table with tablename and for specific cellname,
    if cellname is a list also return value is a list
    """

    if( isinstance(cellname, list) ):
      datatype = []
      for celln in cellname:
        datat  = self.get_type_cell_name_in_deftab(tabname,celln)
        if( self.status != self.OKAY ): return []
        datatype.append(datat)
      #endfor
    else:
      datatype  = self.get_type_cell_name_in_deftab(tabname,cellname)
      if( self.status != self.OKAY ): return 0
    #endif
...
#-------------------------------------------------------------------------------
  def get_tab_data_with_value(self,tabname,cellname,value,header_liste=[]):
    """
    Von Tabelle tabname mit dem Zellenname = value alle Daten der header_liste zurückgeben,
    wenn header_liste leer, dann alles zurückgeben
    tabname: name of table
    cellname: name eine header in der Tabelle
    value:    name des items dass unter den header zu finden ist
...
#-------------------------------------------------------------------------------
#-------------------------------------------------------------------------------
  def get_tab_data_with_value_type_in_dict(self,tabname,cellname,value,header_liste=[]):
    """
    Von Tabelle tabname mit dem Zellenname = value alle Daten und Typen zurückgeben,
    wenn header_liste leer, dann alles zurückgeben
    tabname: name of table
    ...
    """
  	...
  	# Daten und Header holen
    (header_liste,data_liste) = self.get_tab_data_with_value(tabname,cellname,value,header_liste)
    ...
    if( isinstance(value, str) ):
      ...
      for index,header in enumerate(header_liste):
        # Typen holen
        datatype  = self.get_type_cell_name_in_deftab(tabname,header)
        d[header] = [data_liste[0][index],datatype]
      #endfor
    #endif

    return d
  #enddef
  def get_tab_data_with_value_in_dict(self,tabname,cellname,value,header_liste=[]):
    """
    Von Tabelle tabname mit dem Zellenname = value alle Daten zurückgeben,
    wenn header_liste leer, dann alles zurückgeben
    tabname: name of table
    cellname: name eine header in der Tabelle
    value:    name des items dass unter den header zu finden ist
     d = dbh.get_tab_data_with_value_in_dict(tabname,cellname,value)
     c = dbh.get_tab_data_with_value_in_dict(tabname,cellname,value,header_liste)
...
#-------------------------------------------------------------------------------
  def get_type_cell_name_in_deftab(self,tabname,cellname):
    ...
    #                         DB_DATA_TYPE_INT         Integer-Typ
    #                         DB_DATA_TYPE_KEY         Key-Typ       Key einer anderen Tabelle
    #                         DB_DATA_TYPE_CENT        Inter-Typ für Cent
    """
    for tabdef in self.DbDefTab:
      if( tabdef.name == tabname ):
        for celldef in tabdef.cells:
...
#-------------------------------------------------------------------------------
  def is_int_type(self,tabname,cellname):
    ...
    elif( datatype == hdb.DB_DATA_TYPE_CENT ): return True
    else:                                   return False
#-------------------------------------------------------------------------------
#-------------------------------------------------------------------------------
