...
def set_kat(rd,konto_obj,irow):
    """

    :param rd:
    :param konto_obj:
    :param irow:
    :return: (status, konto_obj) = depot_konto_kategorie.set_kat(rd,konto_obj,irow)
    """
    ...
    return (status, konto_obj)
# end def
def kat_regel_build(rd, konto_obj, irow):
    """

    :param rd:
    :param konto_obj:
    :param irow:
    :return: (status, konto_obj) = depot_konto_kategorie.kat_regel_build(rd, konto_obj, irow)
    """
...
def edit_dict(rd):
    ...
    :param rd:
    :return: status = depot_konto_kategorie.edit_dict(rd)
    """
    status = hdef.OKAY
    ...
    if (rd.par.KONTO_KAT_LIST_NAME in ddict_mod.keys()) and \
        ...
        elif len(rd.allg.katfunc.infotext) > 0:
...
#             else:
#                 (status, konto_obj) = set_kat(rd,konto_obj,irow)
#             # end if
#
#             runflag = True
#             index_abfrage = i_go_on
...
#             runflag = True
#
#             (status, konto_obj) = kat_regel_build(rd, konto_obj, irow)
#
#             if status != hdef.OKAY:
#                 index_abfrage = i_go_on
#             else:
